...
The context window has a finite size, measured in **tokens** (roughly ¾ of a word each). [Everything competes for that space](https://platform.claude.com/docs/en/build-with-claude/context-windows): your instructions, the relevant files, the conversation so far, tool outputs, the rules you've set. When people say a model "got dumber" mid-session, what usually happened is the folder got stuffed with noise and the signal got buried — or it overflowed and the earliest, most important pages fell out the back.

So the entire game is: **what goes in the folder, and in what shape?** That's [context engineering](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents). And it has exactly four moves.

## Move 1 — Load: get the right material in

![Two manila folders on a desk — one bulging and overstuffed, one slim with a few clearly-tabbed pages. A tight folder beats a fat one every time.](/img/context-is-king/context-load-tight-vs-fat-folder.png)
...
![A hand pulling one red-flagged contradictory page out of a manila folder while the ordered pages remain — curation as the immune system that removes bad context.](/img/context-is-king/context-curate-immune-system.png)
...
![An automated conveyor line assembling a finished manila folder from labeled input bins — a diff, docs, standards, a ticket — maintaining the machine that builds the context.](/img/context-is-king/context-pipeline-folder-assembly.png)
...
A lot of what goes in the folder is *stable* — your system instructions, the project's ground rules, the big reference doc that doesn't change between turns. Re-sending it every single turn means re-reading (and re-paying for) the same pages over and over. **[Caching](https://platform.claude.com/docs/en/build-with-claude/prompt-caching)** is telling the system "this part of the folder didn't change — reuse it." Done right it cuts both latency and cost dramatically, because the model only processes the new pages, not the whole manila folder from scratch each time.
