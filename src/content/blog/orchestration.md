...
Decompose instead. The same rule that makes for good skill design applies to whole workflows: *if one unit is doing five things, split it into five and write a sixth that orchestrates them.* Each stage gets its own narrow slice of the spec, its own guardrails, its own verification — and the orchestrator drives them in sequence. A pipeline of small, verified steps beats one heroic megaprompt every time, because when something fails you know *which* step, you can re-run *just* that step, and every stage carries its own proof forward. This is how a spec stops being a wish and becomes a program: you give it an execution model.
...
This is the deepest lesson in the whole series, and it's worth stating bluntly: **an ephemeral, unstructured substrate can't hold intent — a durable, version-controlled one can.** The distinction isn't *file vs. conversation*. A scratch note you write once and never open again is just as lossy as a chat log; the plan earns its reliability by being structured, committed to git, and read back. Conversation is RAM — fast, rich, and gone the moment the power blinks. A plan in git is disk: structured, versioned, and still there tomorrow. If you want intent to outlive the session, you have to write it somewhere built to survive the session. The plan *is* the memory.
...
The first is **narrative drift**, the long-session trap. Over a long run, an agent develops a *story* about what's happening, and then starts interpreting every new piece of evidence to fit the story instead of updating it. It decided early the bug was in the auth module, and now every log line is "more evidence" for auth — even as the real problem sits untouched in the database layer. In human cognition this has a name: **confirmation bias** — selectively reading new evidence as support for the hypothesis you already hold instead of revising it. Agents inherit the pattern wholesale, because they're trained on a corpus full of us doing exactly this; narrative drift is confirmation bias running inside a context window. (I dig into that failure mode, and its mechanical cousin context poisoning, in [Garbage In, Gospel Out](/blog/garbage-in-gospel-out/).) The countermeasure is to make the agent argue with itself: for any investigation past ~10 steps, force it to re-state its current hypothesis and list the evidence *for and against* before continuing. Drift loves momentum; this breaks the momentum.
